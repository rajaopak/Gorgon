    ...
    public void start(String mark) {
        if (hasCooldown(mark)) {
            if (cooldowns.get(mark) <= System.currentTimeMillis()) {
                stop(mark);
                ChatUtil.sendMessage(Bukkit.getPlayer(mark), "&cCooldown has been stopped.");
            } else {
                ChatUtil.sendMessage(Bukkit.getPlayer(mark), "&cCooldown is still active. Time left:&e" + timeLeft((int) ((cooldowns.get(mark) - System.currentTimeMillis()) / 1000)));
            }
        }
    }
