...
public class PlayerCooldown {
    ...
    public void start(UUID uuid) {
        if (hasCooldown(uuid)) {
            if (cooldowns.get(uuid) <= System.currentTimeMillis()) {
                stop(uuid);
                ChatUtil.sendMessage(Bukkit.getPlayer(uuid), "&cYou have been removed from the cooldown.");
            } else {
                ChatUtil.sendMessage(Bukkit.getPlayer(uuid), "&cYou are still on cooldown for&e" + timeLeft((int) ((cooldowns.get(uuid) - System.currentTimeMillis()) / 1000)));
            }
        }
    }
